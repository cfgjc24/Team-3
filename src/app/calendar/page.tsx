...
const Schedule: React.FC = () => {
  ...
  const [selectedTime, setSelectedTime] = useState<{ time: string; day: string; count: number } | null>(null);
  const [userSlots, setUserSlots] = useState<{ day: string; time: string }[]>([]); // State to track selected slots
  ...
  const handleSignUp = () => {
    if (selectedTime) {
      ...
      setUserSlots([...userSlots, { day: selectedTime.day, time: selectedTime.time }]);
    }
    setShowForm(false);
  };
...
const timeIntervals = [
  ...
  const convertToBlockData = (dummyData: DummyEvent[], daysOfWeek: string[]): BlockData[] => {
    return dummyData
      .map((event) => {
    ...
    <div className="weekly-schedule">
      ...
      {timeIntervals.map((time, index) => (
        <div className="row" key={index}>
          <div className="time-label">{time}</div> {/* Time labels aligned on the left */}
            ...
            {Array.from({ length: 7 }).map((_, idx) => {
              ...
              return (
                <button
                  ...
                  key={idx}
                  style={{
                    backgroundColor: isSlotSelected(day, time) ? '#9ca3af' : getColorByCount(eventCount),
